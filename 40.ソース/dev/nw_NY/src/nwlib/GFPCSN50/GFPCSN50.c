    ...
    short           *matchinfo_len)
{
    ...
    *matchinfo_len = 0;
    ...
    if (memcmp(mtiinfo, DEF_NY_MTI_0800_REQ, strlen(DEF_NY_MTI_0800_REQ))==0) {
        /* 要求応答マッチングキー生成(要求電文受信) */
        if ((ffd_0800->b011_system_audit_number.ffd_header.m_flg_exist) &&
    ...
    }else if (memcmp(mtiinfo, DEF_NY_MTI_0810_RSP, strlen(DEF_NY_MTI_0810_RSP))==0) {
        /* 要求応答マッチングキー生成(応答電文受信) */
        if ((ffd_0810->b011_system_audit_number.ffd_header.m_flg_exist) &&
            (ffd_0810->b070_nw_mng_code.ffd_header.m_flg_exist)) {
            memcpy(matchkey->mti, DEF_NY_MTI_0800_REQ, sizeof(matchkey->mti));
            memcpy(matchkey->system_audit_number, 
