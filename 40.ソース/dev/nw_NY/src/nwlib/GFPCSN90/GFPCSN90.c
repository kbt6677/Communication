...
/*                               カットオーバー個別処理[NYCE]を行う。        */
/*                                                                           */
/*        AUTHER            ････ HAS                                         */
/*        PROGRAM-CALL      ････                                             */
/*        WRITTEN-DATE      ････ 2025-09-08                                  */
/*                                                                           */
/*        UPDATE            ････                                             */
/*                                                                           */
/*   版  修正者     修正日     修正内容                                      */
/*  ==== ========== ========== ============================================= */
/*  1.0  HAS        2025/09/08 新規作成                                      */
/*                                                                           */
/*****************************************************************************/
...
#include "GFPCSN90.h"                /* NW個別(カットオーバー個別処理)ヘッダ */
...
)
{
    ...
    msg_nyce_def                *pst_biz_msg;
    fixedform_nyce_0800_def     *pst_fxd_0800_col;

    /* -----------------------------------------------------------------------*/
    /* 初期処理                                                               */
    /* -----------------------------------------------------------------------*/
    /* C401 制御電文処理要求 */
    pst_biz_msg =   (msg_nyce_def *)pst_cr401_inf->data_bu.message_text;
    pst_fxd_0800_col = (fixedform_nyce_0800_def *)&pst_biz_msg->ffd;
    ...
    /* BITMAP部精査 */
    if(pst_fxd_0800_col->b007_trans_date_time.ffd_header.m_flg_exist != true){
        /* エラー発生BIT番号設定 */
        memcpy(pch_err_bit,
               DEF_CHK_ERR_BIT_007,
    ...
    if(pst_fxd_0800_col->b011_system_audit_number.ffd_header.m_flg_exist != true){
        /* エラー発生BIT番号設定 */
        memcpy(pch_err_bit,
               DEF_CHK_ERR_BIT_011,
    ...
    if(pst_fxd_0800_col->b070_nw_mng_code.ffd_header.m_flg_exist != true){
        /* エラー発生BIT番号設定 */
        memcpy(pch_err_bit,
               DEF_CHK_ERR_BIT_070,
    ...
    if(pst_fxd_0800_col->b096_msg_security_code.ffd_header.m_flg_exist != false){
        /* エラー発生BIT番号設定 */
        memcpy(pch_err_bit,
               DEF_CHK_ERR_BIT_096,
    ...
    if(pst_fxd_0800_col->b125_nw_mng_info.ffd_header.m_flg_exist != false){
        /* エラー発生BIT番号設定 */
        memcpy(pch_err_bit,
               DEF_CHK_ERR_BIT_125,
    ...
    memcpy(&buf[4],
           pst_fxd_0800_col->b007_trans_date_time.ffd_data,
    ...
    memset( buf, DEF_NWM_CTO_NULL, sizeof(buf));
    memcpy( buf,
            pst_fxd_0800_col->b011_system_audit_number.ffd_data,
            pst_fxd_0800_col->b011_system_audit_number.ffd_header.m_fixvalue_length);
...
)
{
    ...
    /* システム間インターフェース・NYCE・電文ヘッダ */
    msg_nyce_def                *pst_req_biz_msg;
    fixedform_nyce_0800_def     *pst_req_fxd_0800_col;

    /* C401 制御電文処理要求 */
    cr401_def                   *pst_rsp_cr401_inf = (cr401_def *)pch_rspmsg;
    /* システム間インターフェース・NYCE・電文ヘッダ */
    msg_nyce_def                *pst_rsp_biz_msg;
    fixedform_nyce_0810_def     *pst_rsp_fxd_0810_col;
    ...
    *psh_len = (short)(
    ...
    /* 要求 */
    /* C401 制御電文処理要求 */
    pst_req_biz_msg      = (msg_nyce_def *)&pst_req_cr401_inf->data_bu;
    pst_req_fxd_0800_col = (fixedform_nyce_0800_def *)&pst_req_biz_msg->ffd;

    /* 応答 */
    pst_rsp_biz_msg      = (msg_nyce_def *)&pst_rsp_cr401_inf->data_bu;
    pst_rsp_fxd_0810_col = (fixedform_nyce_0810_def *)&pst_rsp_biz_msg->ffd;
    ...
    if (memcmp(pst_rsp_cr401_inf->control_info.response_kind,
        ...
        /* -----------------------------------------------------------------------*/
        /* MTI */
        memcpy(pst_rsp_biz_msg->mti, DEF_NY_MTI_0810_RSP, strlen(DEF_NY_MTI_0810_RSP));

        // 以降は基本的にフラグが有効な場合に要求データコピーで対応する。
        // F7.Transmission Date and Time
        NWM_CTO_MCR_CPY_0810(pst_rsp_fxd_0810_col->b007_trans_date_time,
                             pst_req_fxd_0800_col->b007_trans_date_time);

        // F11.System Trace Audit Number
        NWM_CTO_MCR_CPY_0810(pst_rsp_fxd_0810_col->b011_system_audit_number,
                             pst_req_fxd_0800_col->b011_system_audit_number);

        // F39.アクションコード
        pst_rsp_fxd_0810_col->b039_response_code.ffd_header.m_flg_exist = 1;
        pst_rsp_fxd_0810_col->b039_response_code.ffd_header.m_fixvalue_length =
                      sizeof(pst_rsp_fxd_0810_col->b039_response_code.ffd_data);
        /* 正常応答のみ */
        memcpy(pst_rsp_fxd_0810_col->b039_response_code.ffd_data,
               DEF_NWM_CTO_ACT_INSP_OK,
               strlen(DEF_NWM_CTO_ACT_INSP_OK));

        // F70.Network Management Information Code
        NWM_CTO_MCR_CPY_0810(pst_rsp_fxd_0810_col->b070_nw_mng_code,
                             pst_req_fxd_0800_col->b070_nw_mng_code);
    }
    return;
}
/*****************************************************************************/
